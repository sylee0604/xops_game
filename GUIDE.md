# XOPS GAME — AI REFERENCE (2026-04-02)
...
js/audio.js          getAudioCtx(), playGunshot(idx), playEnemyGunshot(idx)
                     playPlayerStep(crouching), playEnemyStep(distSq)
...
js/enemies.js        canSeePlayer(e), spawnEnemies(), spawnEnemyAt(x,z,wpList,isZombie,floorY=0)
                     spawnAllyAt(x,z), updateEnemies(dt), updateAllies(dt)
                     enemyShoot(e), allyShoot(ally,targetEnemy)
                     hitEnemy(e,dmg,fromAlly), killEnemy(e,fromAlly), hitAlly(a,dmg), killAlly(a)
js/instanced.js      InstancedMesh 렌더러 — ITYPE{SOLDIER:0,ZOMBIE:1,ALLY:2}
                     EnemyRenderer.init(scene), .allocate(type), .free(idx,type)
                     .updateAll(enemies,allies), .flashHit(idx,type)
                     .setDeadPose(idx,type,pos,facing,baseY), .reset()
...
js/maps/rescue.js    buildRescueMap(), spawnRescueEnemies(), updateRescue(dt), onRescueAllDead()
js/maps/defuse.js    buildDefuseMap(), spawnDefuseEnemies(), updateDefuse(dt)
...
→ pickups → enemies → instanced → bullets → weapons → hud → player
→ maps/combat → maps/harbor → maps/tunnel → maps/training → maps/assault
→ maps/survival → maps/rescue → maps/defuse → game
```
...
## ENEMY 객체 (instanced.js 전환 후)
```js
{
  instanceIdx,    // EnemyRenderer 인스턴스 슬롯 번호 (-1이면 비활성)
  instanceType,   // ITYPE.SOLDIER | ITYPE.ZOMBIE | ITYPE.ALLY
  ...
  stepTimer,      // 다음 발소리까지 남은 시간
  isZombie,
  waypoints[], wpIndex,
  facing,         // 라디안
  walkPhase,      // 보행 bob 위상 (EnemyRenderer.updateAll에서 읽음)
  attackTarget,   // null | ally 객체
}
// ※ mesh / bodyMesh 필드 없음 — InstancedMesh로 전환됨
```
...
## InstancedMesh 렌더러 (instanced.js)

### 개요
모든 적/아군 모델을 InstancedMesh로 렌더링. 적 수에 관계없이 고정 ~55 draw call.
- SOLDIER 풀: 최대 50 인스턴스, 22개 파트 (몸+총)
- ZOMBIE 풀: 최대 30 인스턴스, 14개 파트 (팔 앞으로 뻗음, 총 없음)
- ALLY 풀: 최대 8 인스턴스, 20개 파트

### 공유 재질 전략
```
3개 white(0xffffff) 재질 공유:
  _matB — MeshPhong body (일반 몸)
  _matE — MeshBasic eye (눈)
  _matG — MeshPhong gun (shininess:60)
실제 색상은 instanceColor로 제어.
```

### 색상 제어
- 스폰 시: `ICOLORS[type][colorKey]` 기본 색으로 초기화
- 피격 flash: `flashHit(idx, type)` → vest 빨간색 80ms 후 복원
- 사망: `setDeadPose(idx, type, pos, facing, baseY)` → 전 파트 0x111111
- 숨기기: `_IHIDDEN = Matrix4.makeScale(0.001,0.001,0.001)` (near-zero scale)

### 사용 패턴
```js
// 스폰 시
const iIdx = EnemyRenderer.allocate(ITYPE.SOLDIER); // 또는 ZOMBIE/ALLY
enemies.push({ instanceIdx: iIdx, instanceType: ITYPE.SOLDIER, ... });

// 피격 시
EnemyRenderer.flashHit(e.instanceIdx, e.instanceType);

// 사망 시
EnemyRenderer.setDeadPose(e.instanceIdx, e.instanceType, e.pos, e.facing, e.baseY);
...
### gameLoop 연동
```
startGame() → EnemyRenderer.init(scene)   ← initThree() 직후
gameLoop()  → EnemyRenderer.updateAll(enemies, allies)  ← updateEnemies(dt) 직후
```
...
```
`startGame()` 내부에서 `EnemyRenderer.init(scene)` → `MAP_REGISTRY[currentMap]?.setup()` 호출 후
`GameEvents.off('all_enemies_dead')` → `GameEvents.on('all_enemies_dead', mapDef.onAllDead)` 등록.
`gameLoop()` 내부에서 `EnemyRenderer.updateAll(enemies, allies)` → `MAP_REGISTRY[currentMap]?.update?.(dt)` 호출.
...
## 발소리 시스템 (audio.js + player.js + enemies.js)
```
playPlayerStep(crouching)
  — Web Audio 절차적 합성 (노이즈 버스트 + LPF 170Hz)
  — 달리기 0.38초 간격, 앉기 0.60초 간격
  — 볼륨: 달리기 0.30 / 앉기 0.12
  — 플레이어 이동 중 + onGround 조건에서만 재생 (player.js _stepTimer)

playEnemyStep(distSq)
  — 거리 15유닛 이상 무음, 거리 비례 볼륨 (최대 0.18)
  — 적 AI와 무관 (청각 감지 없음)
  — enemies.js updateEnemies에서 e.stepTimer로 0.40초 간격 호출
```
...
웨이브 초기화: EnemyRenderer.reset() + enemies.length = 0
```
...
```
initThree()
→ EnemyRenderer.init(scene)        ← InstancedMesh 풀 생성
→ createWeaponModels() → setupInput() → getAudioCtx()
...
## gameLoop() 업데이트 순서
```
updatePlayer → updateEnemies → EnemyRenderer.updateAll(enemies,allies)
...
**2.** `index.html` `_GAME_FILES` 배열에 `'js/maps/newmap.js'` 추가 (defuse.js 앞, game.js 앞)
...
죽은 적 3초 후 free                → killEnemy() setTimeout 안에서
                                     EnemyRenderer.free(idx,type) + e.instanceIdx=-1 + enemies.splice
적 렌더링                          → InstancedMesh (instanced.js)
                                     mesh/bodyMesh 직접 조작 금지
                                     피격=flashHit / 사망=setDeadPose / 제거=free
```
